...
def ai_worker():
    while True:
        ...
        send_alert(label, conf, x)

Thread(target=capture, args=(packet_queue,), daemon=True).start()
for _ in range(4):
    Thread(target=flow_worker, daemon=True).start()
for _ in range(2):
    Thread(target=session_worker, daemon=True).start()
